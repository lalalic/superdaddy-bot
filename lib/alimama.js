...
function login(alimama_username,alimama_password, browser){
	...
	.then(loaded=>console.log(`login url launched`))
        ...
        .then(a=>browser.findElement(By.id('J_SubmitStatic')).click())
	.then(loaded=>console.log(`user account submitted to login`))
        .then(a=>browser.wait(until.urlContains("www.alimama.com"), 2*timeout,"should go to alimama"))
        .then(a=>browser.wait(until.elementLocated(By.css("a[href*='spm=']")), 2*timeout,"should go to alimama"))
