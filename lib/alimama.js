...
function convertUrl(url,spm, browser){
    ...
    .then(a=>console.log(`> 提交url`))
        .then(a=>browser.wait(until.elementLocated(By.css("#vf-dialog button")), timeout, "convertor form should be visible"))
    .then(a=>console.log("－－参数设置界面"))
        .then(a=>Promise.all([// 设置参数
            browser.findElements(By.css("form#J_zone_add input"))
                .then(elements=>{
                    console.log(`found ${elements.length} input`)
                    elements.forEach(a=>a.getAttribute("value").then(a=>console.log(`input=${a}`)))
                }),//**@TODO: it will make call always success
