...
function getPhantomDriver(){
	...
	driver.manage().timeouts().pageLoadTimeout(6*timeout)
...
function login(alimama_username,alimama_password, browser){
	...
	.then(loaded=>console.log(`login url launched`))/*
		.then(loaded=>browser.executeScript(function(username,password){
			document.querySelector("input[name=TPL_username]").value=username
			document.querySelector("input[name=TPL_password]").value=password
			document.querySelector("#J_SubmitStatic").click()
		}),alimama_username, alimama_password)*/
...
convert(itemUrl,{"推广类型":"网站推广","导购名称":"wechat","推广位名称":"个人推荐"})
